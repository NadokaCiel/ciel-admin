  ...
  <div tabindex="-1" class="slider-2048" @keyup.up="change('up')" @keyup.down="change('down')" @keyup.left="change('left')" @keyup.right="change('right')">
    ...
    <transition-group name="cell" tag="div" class="box game-board" :style="{'width':(width) * 50 + 'px','height':(height) * 50 + 'px'}">
      <div v-for="item in graph" class="item-box" :class="['dot-'+item.weight]" :key="item.id">
        <div>{{item.weight ? 2 ** item.weight : ''}}</div>
      </div>
    </transition-group>
    <div class="box">
      <div>Score:{{score}}</div>
...
export default {
  ...
  methods: {
    ...
    newNum() {
      const vm = this;
      const x = Math.floor((Math.random() * vm.width));
      const y = Math.floor((Math.random() * vm.height));
      ...
      if (key === "up") {
        for (let i = 0; i < y; i += 1) {
          if (vm.pointMap[`(${x},${i})`].weight === 0 && vm.clearPath(x, i, x, y)) {
            ...
            vm.changed = true;
            return;
          } if (vm.pointMap[`(${x},${i})`].weight === vm.pointMap[`(${x},${y})`].weight && vm.clearPath(x, i, x, y)) {
            vm.pointMap[`(${x},${y})`].weight = 0;
            vm.pointMap[`(${x},${i})`].weight += 1;
            vm.score += 2 ** vm.pointMap[`(${x},${i})`].weight;
            vm.changed = true;
            return;
          }
        }
      } else if (key === "down") {
        for (let i = vm.height - 1; i > y; i -= 1) {
          if (vm.pointMap[`(${x},${i})`].weight === 0 && vm.clearPath(x, i, x, y)) {
            [vm.pointMap[`(${x},${i})`].weight, vm.pointMap[`(${x},${y})`].weight] = [vm.pointMap[`(${x},${y})`].weight, vm.pointMap[`(${x},${i})`].weight];
            vm.changed = true;
            return;
        ...
        for (let i = 0; i < x; i += 1) {
          if (vm.pointMap[`(${i},${y})`].weight === 0 && vm.clearPath(i, y, x, y)) {
            ...
            vm.changed = true;
            return;
          } if (vm.pointMap[`(${i},${y})`].weight === vm.pointMap[`(${x},${y})`].weight && vm.clearPath(i, y, x, y)) {
            vm.pointMap[`(${x},${y})`].weight = 0;
            vm.pointMap[`(${i},${y})`].weight += 1;
            vm.score += 2 ** vm.pointMap[`(${i},${y})`].weight;
            vm.changed = true;
            return;
          }
        }
      } else if (key === "right") {
        for (let i = vm.width - 1; i > x; i -= 1) {
          if (vm.pointMap[`(${i},${y})`].weight === 0 && vm.clearPath(i, y, x, y)) {
            [vm.pointMap[`(${i},${y})`].weight, vm.pointMap[`(${x},${y})`].weight] = [vm.pointMap[`(${x},${y})`].weight, vm.pointMap[`(${i},${y})`].weight];
            vm.changed = true;
            return;
    ...
    change(key) {
      ...
      if (key === "up") {
        ...
        for (let x = 0; x < vm.width; x += 1) {
          for (let y = vm.height - 1; y >= 0; y -= 1) {
            vm.excute(x, y, key);
          }
        }
      } else if (key === "left") {
        for (let y = 0; y < vm.height; y += 1) {
          for (let x = vm.width - 1; x >= 0; x -= 1) {
            vm.excute(x, y, key);
          }
        }
      } else if (key === "right") {
    ...
    overCheck() {
      const vm = this;
      return vm.graph.every(item => item.weight > 0);
    },
